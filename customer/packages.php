...
document.addEventListener('DOMContentLoaded', function() {
    ...
    document.getElementById('traditionalBookingForm').addEventListener('submit', function(e) {
        ...
        // Get date values
        const dateOfBirth = document.getElementById('traditionalDateOfBirth').value;
        const dateOfDeath = document.getElementById('traditionalDateOfDeath').value;
        const dateOfBurial = document.getElementById('traditionalDateOfBurial').value;
        const today = new Date();
        ...
        if (dateOfDeath) {
            ...
            // If date of birth is provided, death date must be after birth date
            if (dateOfBirth && deathDate < new Date(dateOfBirth)) {
                Swal.fire({
                    title: 'Invalid Date',
                    text: 'Date of Death cannot be before Date of Birth',
                    icon: 'error',
                    confirmButtonColor: '#d97706'
                });
                return false;
        ...
        if (dateOfBurial) {
            const burialDate = new Date(dateOfBurial);
            const minBurialDate = new Date();
            minBurialDate.setDate(today.getDate() - 1); // Allow yesterday to account for timezone differences
            
            // Burial date cannot be before death date
            if (dateOfDeath && burialDate < new Date(dateOfDeath)) {
                Swal.fire({
                    title: 'Invalid Date',
            ...
            // Burial date should not be too far in the past (more than 1 year)
            const oneYearAgo = new Date();
            oneYearAgo.setFullYear(oneYearAgo.getFullYear() - 1);
            
            if (burialDate < oneYearAgo) {
                Swal.fire({
                    title: 'Invalid Date',
                    text: 'Date of Burial cannot be more than one year in the past',
                    icon: 'error',
                    confirmButtonColor: '#d97706'
                });
                return false;
