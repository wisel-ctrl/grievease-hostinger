...
if ($booking_type === 'traditional') {
    ...
    // Build count query for Custom Package bookings
    $count_query = "SELECT COUNT(*) as total FROM custom_booking_tb WHERE customer_id = ?";
    if ($status !== 'all') {
        $count_query .= " AND status = ?";
    }
    $stmt = $conn->prepare($count_query);
    ...
    // Build main query for Custom Package bookings
    ...
    $query .= " ORDER BY cb.booking_date " . ($sort === 'newest' ? 'DESC' : 'ASC') . " LIMIT ? OFFSET ?";
    $stmt = $conn->prepare($query);
    if ($status !== 'all') {
        $stmt->bind_param("isii", $user_id, $status, $items_per_page, $offset);
    } else {
    ...
    $custom_bookings = [];
    while ($booking = $result->fetch_assoc()) {
        $custom_bookings[] = $booking;
    }
    $stmt->close();

    // Generate HTML for Custom Package bookings
    ob_start();
    if (count($custom_bookings) > 0) {
            ...
            <div class="bg-white rounded-xl border border-gray-200 shadow-sm overflow-hidden mb-4">
                ...
                <div class="p-4 sm:p-6 space-y-3 sm:space-y-4">
                    <div class="grid grid-cols-1 md:grid-cols-3 gap-4 mb-3">
                        <div>
                            <p class="text-sm text-gray-500">Deceased Name</p>
                            <p class="text-purple-600"><?php echo htmlspecialchars(ucwords(strtolower($deceased_name))); ?></p>
                        </div>
                        <div>
                            <p class="text-sm text-gray-500">Branch</p>
                            <p class="text-purple-600"><?php echo htmlspecialchars(ucwords(strtolower($booking['branch_name']))); ?></p>
                        </div>
                        <div>
                            <p class="text-sm text-gray-500">Service Date</p>
                            <p class="text-purple-600"><?php echo htmlspecialchars($service_date); ?></p>
                        </div>
                    </div>
                    <div class="grid grid-cols-1 md:grid-cols-3 gap-4 mb-3">
                        <div>
                            <p class="text-sm text-gray-500">Total Amount</p>
                            <p class="text-purple-600 font-bold">₱<?php echo htmlspecialchars($total_price); ?></p>
                        </div>
                        <div>
                            <p class="text-sm text-gray-500">Amount Paid</p>
                            <p class="text-purple-600">₱<?php echo htmlspecialchars($amount_paid); ?></p>
                        </div>
                        <div>
                            <p class="text-sm text-gray-500">Balance</p>
                            <p class="text-purple-600">₱<?php echo htmlspecialchars($balance); ?></p>
                        </div>
                    </div>
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                        <div>
                            <p class="text-sm text-gray-500">Package Details</p>
                            <p class="text-purple-600"><?php echo htmlspecialchars($booking['package_details'] ?? 'Custom Package'); ?></p>
                        </div>
                        <div>
                            <p class="text-sm text-gray-500">Booking Date</p>
                            <p class="text-purple-600"><?php echo htmlspecialchars($booking_date); ?></p>
                        </div>
                    </div>
                    <div class="flex justify-end">
