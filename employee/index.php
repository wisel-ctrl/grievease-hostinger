...
.main-content {
  margin-left: 16rem; /* Matches the sidebar width on desktop */
...
/* Utilities used by JS toggle */
.w-0 { width: 0; }
.opacity-0 { opacity: 0; }
.invisible { visibility: hidden; }
.w-\[calc\(100\%-16rem\)\] { width: calc(100% - 16rem); }
.w-\[calc\(100\%-4rem\)\] { width: calc(100% - 4rem); }

/* Responsive override: on screens smaller than lg, main content should be full-width */
...
<?php include 'employee_sidebar.php'; ?>
  ...
  <!-- Main Content -->
<div id="main-content" class="lg:ml-64 ml-0 p-3 sm:p-4 lg:p-6 bg-gray-50 min-h-screen transition-all duration-300 main-content">
  <!-- Header with breadcrumb and welcome message -->
  <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-4 sm:mb-6 bg-white p-3 sm:p-5 rounded-lg shadow-sidebar">
    <div class="mb-3 sm:mb-0">
      <h1 class="text-xl sm:text-2xl font-bold text-sidebar-text">Employee Dashboard</h1>
      <p class="text-sm text-gray-500">Welcome back, </p>
    </div>
    <div class="flex space-x-3">
      <!-- Mobile menu button -->
      <button id="mobile-menu-btn" class="lg:hidden p-2 bg-sidebar-accent text-white rounded-lg shadow-md hover:bg-darkgold transition-all duration-300">
  ...
  <!-- Quick Stats -->
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3 sm:gap-4 lg:gap-5">
      <!-- Services this Month -->
      <div class="bg-white rounded-lg shadow-sidebar p-3 sm:p-4 lg:p-5 border border-sidebar-border hover:shadow-card transition-all duration-300">
        <div class="flex items-center mb-3">
          <div class="w-10 h-10 sm:w-12 sm:h-12 rounded-lg bg-blue-100 text-blue-600 flex items-center justify-center mr-3 flex-shrink-0">
            <i class="fas fa-calendar-alt text-sm sm:text-lg"></i>
          </div>
          <span class="text-sidebar-text font-medium text-sm sm:text-base">Services this Month</span>
        </div>
        <div class="text-2xl sm:text-3xl font-bold mb-2 text-sidebar-text"><?php echo $services_this_month; ?></div>
        <div class="text-xs sm:text-sm text-green-600 flex items-center">
      ...
      <div class="bg-white rounded-lg shadow-sidebar p-3 sm:p-4 lg:p-5 border border-sidebar-border hover:shadow-card transition-all duration-300">
        <div class="flex flex-col sm:flex-row items-start sm:items-center justify-between mb-3 gap-2">
          <div class="flex items-center">
            <div class="w-10 h-10 sm:w-12 sm:h-12 rounded-lg bg-green-100 text-green-600 flex items-center justify-center mr-3 flex-shrink-0">
              <i class="fas fa-peso-sign text-sm sm:text-lg"></i>
            </div>
            <span class="text-sidebar-text font-medium text-sm sm:text-base">Monthly Revenue</span>
          </div>
          <div class="relative">
            <button id="revenue-toggle" class="p-1 bg-gray-100 rounded-full flex items-center text-xs">
              <span id="revenue-type" class="px-2">Cash</span>
              <i class="fas fa-chevron-down mr-1"></i>
            </button>
            <div id="revenue-dropdown" class="absolute right-0 mt-1 w-20 sm:w-24 bg-white rounded-md shadow-lg hidden z-10">
              <button class="block w-full text-left px-3 py-2 text-xs sm:text-sm text-gray-700 hover:bg-gray-100" onclick="toggleRevenue('cash')">Cash</button>
              <button class="block w-full text-left px-3 py-2 text-xs sm:text-sm text-gray-700 hover:bg-gray-100" onclick="toggleRevenue('accrual')">Accrual</button>
            </div>
          </div>
        </div>
        <div id="cash-revenue" class="text-2xl sm:text-3xl font-bold mb-2 text-sidebar-text">₱<?php echo number_format($cash_revenue, 2); ?></div>
        <div id="accrual-revenue" class="text-2xl sm:text-3xl font-bold mb-2 text-sidebar-text hidden">₱<?php echo number_format($accrual_revenue, 2); ?></div>
        <div class="text-xs sm:text-sm text-green-600 flex items-center">
      ...
      <div class="bg-white rounded-lg shadow-sidebar p-3 sm:p-4 lg:p-5 border border-sidebar-border hover:shadow-card transition-all duration-300">
        <div class="flex items-center mb-3">
          <div class="w-10 h-10 sm:w-12 sm:h-12 rounded-lg bg-orange-100 text-orange-600 flex items-center justify-center mr-3 flex-shrink-0">
            <i class="fas fa-tasks text-sm sm:text-lg"></i>
          </div>
          <span class="text-sidebar-text font-medium text-sm sm:text-base">Ongoing Services</span>
        </div>
        <div class="text-2xl sm:text-3xl font-bold mb-2 text-sidebar-text"><?php echo $ongoing_services; ?></div>
        <div class="text-xs sm:text-sm text-red-600 flex items-center">
  ...
  <div class="bg-white rounded-lg shadow-sidebar border border-sidebar-border hover:shadow-card transition-all duration-300 mb-6 sm:mb-8">
    <div class="flex justify-between items-center p-3 sm:p-5 border-b border-sidebar-border">
      <h3 class="font-medium text-sidebar-text text-sm sm:text-base">Pending Bookings</h3>
    </div>
    <div class="overflow-x-auto scrollbar-thin">
      <table class="w-full">
        <thead>
          <tr class="bg-sidebar-hover">
            <th class="p-2 sm:p-4 text-left text-xs sm:text-sm font-medium text-sidebar-text cursor-pointer" onclick="sortTable(0)">
              <div class="flex items-center">
                Client Name <i class="fas fa-sort ml-1 text-gray-400 hidden sm:inline"></i>
              </div>
            </th>
            <th class="p-2 sm:p-4 text-left text-xs sm:text-sm font-medium text-sidebar-text cursor-pointer hidden md:table-cell" onclick="sortTable(1)">
              <div class="flex items-center">
                Service Type <i class="fas fa-sort ml-1 text-gray-400"></i>
              </div>
            </th>
            <th class="p-2 sm:p-4 text-left text-xs sm:text-sm font-medium text-sidebar-text cursor-pointer" onclick="sortTable(2)">
              <div class="flex items-center">
                Date <i class="fas fa-sort ml-1 text-gray-400 hidden sm:inline"></i>
              </div>
            </th>
            <th class="p-2 sm:p-4 text-left text-xs sm:text-sm font-medium text-sidebar-text cursor-pointer hidden lg:table-cell" onclick="sortTable(3)">
              <div class="flex items-center">
                Location <i class="fas fa-sort ml-1 text-gray-400"></i>
              </div>
            </th>
            <th class="p-2 sm:p-4 text-left text-xs sm:text-sm font-medium text-sidebar-text cursor-pointer" onclick="sortTable(4)">
              <div class="flex items-center">
                Status <i class="fas fa-sort ml-1 text-gray-400 hidden sm:inline"></i>
              </div>
            </th>
          </tr>
        </thead>
        <tbody>
          ...
          if ($pending_result->num_rows > 0) {
            while ($booking = $pending_result->fetch_assoc()) {
              ...
              echo '<td class="p-2 sm:p-4 text-xs sm:text-sm text-sidebar-text">';
              echo '<div class="font-medium">' . htmlspecialchars($booking['full_name']) . '</div>';
              echo '<div class="text-xs text-gray-500 md:hidden">' . htmlspecialchars($booking['service_name']) . '</div>';
              echo '</td>';
              echo '<td class="p-2 sm:p-4 text-xs sm:text-sm text-sidebar-text hidden md:table-cell">' . htmlspecialchars($booking['service_name']) . '</td>';
              echo '<td class="p-2 sm:p-4 text-xs sm:text-sm text-sidebar-text">';
              echo '<div>' . date('M j, Y', strtotime($booking['booking_date'])) . '</div>';
              echo '<div class="text-xs text-gray-500 lg:hidden">' . htmlspecialchars($booking['deceased_address']) . '</div>';
              echo '</td>';
              echo '<td class="p-2 sm:p-4 text-xs sm:text-sm text-sidebar-text hidden lg:table-cell">' . htmlspecialchars($booking['deceased_address']) . '</td>';
              echo '<td class="p-2 sm:p-4 text-xs sm:text-sm">';
            ...
            echo '<tr class="border-b border-sidebar-border hover:bg-sidebar-hover">';
            echo '<td colspan="5" class="p-2 sm:p-4 text-xs sm:text-sm text-sidebar-text text-center">No pending bookings found</td>';
            echo '</tr>';
          }
          ?>
        </tbody>
    ...
    <div class="p-3 sm:p-4 border-t border-sidebar-border flex justify-between items-center">
  ...
  <div class="bg-white rounded-lg shadow-sidebar border border-sidebar-border hover:shadow-card transition-all duration-300 mb-6 sm:mb-8">
      <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3 p-3 sm:p-5 border-b border-sidebar-border">
          <h3 class="font-medium text-sidebar-text text-sm sm:text-base">Recent Inventory Activity</h3>
      ...
      <div class="overflow-x-auto scrollbar-thin">
          <table class="w-full">
              <thead>
                  <tr class="bg-sidebar-hover">
                      <th class="p-2 sm:p-4 text-left text-xs sm:text-sm font-medium text-sidebar-text">Item</th>
                      <th class="p-2 sm:p-4 text-left text-xs sm:text-sm font-medium text-sidebar-text hidden sm:table-cell">ID</th>
                      <th class="p-2 sm:p-4 text-left text-xs sm:text-sm font-medium text-sidebar-text">Action</th>
                      <th class="p-2 sm:p-4 text-left text-xs sm:text-sm font-medium text-sidebar-text hidden md:table-cell">Date</th>
                      <th class="p-2 sm:p-4 text-left text-xs sm:text-sm font-medium text-sidebar-text">Quantity</th>
                  </tr>
              </thead>
              <tbody id="inventoryLogsBody">
                  <!-- Loading indicator row -->
                  <tr id="inventoryLoadingIndicator" class="border-b border-sidebar-border">
                      <td colspan="5" class="p-2 sm:p-4 text-xs sm:text-sm text-center text-sidebar-text">
      ...
      <div class="p-3 sm:p-4 border-t border-sidebar-border flex flex-col sm:flex-row justify-between items-start sm:items-center gap-2">
        ...
        .then(data => {
            if (data.success) {
                ...
                data.logs.forEach(log => {
                    ...
                    row.innerHTML = `
                        <td class="p-2 sm:p-4 text-xs sm:text-sm text-sidebar-text">
                            <div class="font-medium">${log.item_name}</div>
                            <div class="mt-1 flex flex-wrap items-center gap-2 sm:hidden">
                                <span class="inline-block bg-gray-100 rounded-full px-2 py-0.5 text-[10px] font-medium">ID: ${log.inventory_id}</span>
                                <span class="inline-flex items-center px-2 py-0.5 rounded-full text-[10px] font-medium ${badgeClass} border">
                                    <i class="fas ${badgeIcon} mr-1"></i> ${log.activity_type}
                                </span>
                                <span class="text-[10px] text-gray-500">${formattedDate}</span>
                            </div>
                        </td>
                        <td class="p-2 sm:p-4 text-xs sm:text-sm text-sidebar-text hidden sm:table-cell">
                        ...
                        <td class="p-2 sm:p-4 text-xs sm:text-sm text-sidebar-text hidden md:table-cell">${formattedDate}</td>
                        <td class="p-2 sm:p-4 text-xs sm:text-sm font-medium text-sidebar-text">
                            ${quantityDisplay}
                        </td>
                    `;
