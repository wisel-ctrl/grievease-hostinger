...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Archive modal search functionality
    const archivedLifePlansSearch = document.getElementById('archivedLifePlansSearch');
    archivedLifePlansSearch.addEventListener('input', function() {
        const searchValue = this.value.toLowerCase();
        const rows = archivedLifePlansBody.querySelectorAll('tr');
        let anyVisible = false;
        rows.forEach(row => {
            // Skip the loading or empty row
            if (row.querySelectorAll('td').length !== 6) return;
            const beneficiary = row.cells[0].textContent.toLowerCase();
            ...
            const matches = beneficiary.includes(searchValue) || service.includes(searchValue) || duration.includes(searchValue) || price.includes(searchValue) || status.includes(searchValue);
            row.style.display = matches ? '' : 'none';
            if (matches) anyVisible = true;
        });
        // Show 'No results' if nothing matches
        if (!anyVisible) {
            archivedLifePlansBody.innerHTML += `<tr><td colspan='6' class='px-6 py-4 whitespace-nowrap text-center text-sm text-gray-500'>No matching archived lifeplans found</td></tr>`;
        } else {
            // Remove any previous 'No matching...' row
            archivedLifePlansBody.querySelectorAll('tr').forEach(row => {
                if (row.textContent.includes('No matching archived lifeplans found')) row.remove();
            });
        }
    });
});
