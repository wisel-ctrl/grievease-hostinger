...
<div id="main-content" class="p-6 bg-gray-50 min-h-screen transition-all duration-300 ml-64 w-[calc(100%-16rem)] main-content">
  <div class="flex justify-between items-center mb-6 bg-white p-4 rounded-lg shadow">
    <h1 class="text-2xl font-cinzel font-bold text-primary">Customer Messages</h1>
    <div class="flex gap-2">
      <div class="relative">
        <input type="text" id="customer-search" placeholder="Search customers..." class="px-4 py-2 pl-10 border border-input-border rounded-md shadow-input focus:outline-none focus:ring-2 focus:ring-gold focus:border-transparent font-hedvig">
        <i class="fas fa-search absolute left-3 top-3 text-gray-400"></i>
      </div>
      <button id="refresh-messages" class="bg-secondary hover:bg-gray-200 text-secondary-foreground px-4 py-2 rounded-md flex items-center gap-2 transition-colors duration-300 font-hedvig">
        <i class="fas fa-sync-alt"></i> Refresh
      </button>
      <div class="relative">
        <button id="filter-dropdown-btn" class="bg-secondary hover:bg-gray-200 text-secondary-foreground px-4 py-2 rounded-md flex items-center gap-2 transition-colors duration-300 font-hedvig">
          <i class="fas fa-filter"></i> Filter
        </button>
        <div id="filter-dropdown" class="hidden absolute right-0 mt-2 w-48 bg-white rounded-md shadow-card z-10">
          <div class="py-1">
            <button class="block w-full text-left px-4 py-2 text-sm text-secondary-foreground hover:bg-secondary transition-colors duration-300 font-hedvig" onclick="filterMessages('all')">All Messages</button>
            <button class="block w-full text-left px-4 py-2 text-sm text-secondary-foreground hover:bg-secondary transition-colors duration-300 font-hedvig" onclick="filterMessages('unread')">Unread Only</button>
            <button class="block w-full text-left px-4 py-2 text-sm text-secondary-foreground hover:bg-secondary transition-colors duration-300 font-hedvig" onclick="filterMessages('today')">Today</button>
            <button class="block w-full text-left px-4 py-2 text-sm text-secondary-foreground hover:bg-secondary transition-colors duration-300 font-hedvig" onclick="filterMessages('week')">This Week</button>
            <button class="block w-full text-left px-4 py-2 text-sm text-secondary-foreground hover:bg-secondary transition-colors duration-300 font-hedvig" onclick="filterMessages('month')">This Month</button>
          </div>
        </div>
      </div>
    </div>
  ...
  <div class="bg-white rounded-lg shadow-md overflow-hidden border border-border hover:shadow-card transition-all duration-300">
    <!-- Messages Header -->
    <div class="border-b border-border bg-secondary p-4">
      <div class="flex justify-between items-center">
        <h2 class="text-lg font-cinzel font-semibold text-primary">Incoming Customer Messages</h2>
        <div class="text-sm text-gray-500 font-hedvig">Showing <span id="message-count" class="font-medium">0</span> messages</div>
      </div>
    </div>
    ...
    <div class="divide-y divide-border">
      ...
      <div id="empty-state" class="py-12 flex flex-col items-center justify-center text-gray-500">
        <div class="bg-secondary rounded-full p-4 mb-4">
          <i class="fas fa-inbox text-3xl"></i>
        </div>
        <h3 class="text-lg font-cinzel font-medium mb-1">No customer messages</h3>
        <p class="text-sm font-hedvig">Customer messages will appear here when received</p>
        <button id="load-messages-btn" class="mt-4 bg-gold hover:bg-darkgold text-white px-4 py-2 rounded-md transition-colors duration-300 font-hedvig">
          Load Messages
        </button>
      </div>
...
<div id="message-detail-modal" class="hidden fixed inset-0 bg-black bg-opacity-50 z-50 flex justify-center items-center">
  <div class="bg-white rounded-lg shadow-card w-full max-w-4xl max-h-[80vh] flex flex-col">
    <!-- Modal Header -->
    <div class="p-4 border-b border-border flex justify-between items-center bg-secondary">
      <div>
        <h3 class="text-lg font-cinzel font-semibold" id="modal-customer-name">Customer Name</h3>
        <p class="text-sm text-gray-500 font-hedvig" id="modal-message-date">Date</p>
      </div>
      <button id="close-modal" class="text-gray-500 hover:text-primary transition-colors duration-300">
        <i class="fas fa-times text-xl"></i>
      </button>
    </div>
    
    <!-- Modal Body - Conversation -->
    <div class="p-4 overflow-y-auto flex-grow scrollbar-thin" id="modal-conversation">
    ...
    <div class="p-4 border-t border-border bg-white">
      <div class="flex gap-2 mb-2">
        <button class="text-gray-500 hover:text-gold transition-colors duration-300" title="Attach File">
          <i class="fas fa-paperclip"></i>
        </button>
        <button class="text-gray-500 hover:text-gold transition-colors duration-300" title="Quick Reply Template">
          <i class="fas fa-reply-all"></i>
        </button>
        <button class="text-gray-500 hover:text-gold transition-colors duration-300" title="Format Text">
          <i class="fas fa-font"></i>
        </button>
      </div>
      <div class="flex gap-2">
        <textarea id="reply-input" class="flex-1 p-2.5 border border-input-border rounded text-sm font-hedvig shadow-input focus:outline-none focus:ring-2 focus:ring-gold focus:border-transparent" placeholder="Type your reply..."></textarea>
        <button class="bg-gold hover:bg-darkgold text-white px-4 py-2 rounded-md transition-colors duration-300 font-hedvig" id="send-reply">
