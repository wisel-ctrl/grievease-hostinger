...
<div class="grid grid-cols-1 gap-4 mb-6">
  ...
  <div class="bg-white rounded-xl shadow-sm hover:shadow-md transition-all duration-300 overflow-hidden">
      ...
      <div class="bg-gradient-to-r from-blue-100 to-blue-200 px-6 py-4">
          <div class="flex items-center justify-between mb-1">
              <h3 class="text-sm font-medium text-gray-700">Sales Forecast (Next 6 Months)</h3>
              <button class="print-revenue-btn w-10 h-10 rounded-full bg-white/90 text-blue-600 flex items-center justify-center hover:bg-blue-50" title="Print/Export">
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Use only one method - direct event listener
    const printBtn = document.querySelector('.print-revenue-btn');
    if (printBtn) {
        console.log('Found print button directly');
        printBtn.addEventListener('click', function(e) {
            console.log('Print button clicked');
            if (typeof printRevenueTables === 'function') {
                printRevenueTables();
            } else {
                console.error('printRevenueTables function not found');
            }
            e.preventDefault();
        });
    } else {
    ...
    // Remove the event delegation to avoid duplicate calls
});

function printRevenueTables() {
    console.log('Print Revenue Tables function called');
    ...
    // Create print content dynamically instead of modifying existing DOM
    const printContent = `
        <div style="font-family: Arial, sans-serif; max-width: 800px; margin: 0 auto;">
            <!-- Header -->
            <div style="text-align: center; margin-bottom: 20px; border-bottom: 2px solid #3A57E8; padding-bottom: 10px;">
                ...
                <p style="color: #666; margin-top: 0;">Generated on: ${new Date().toLocaleString('en-PH', {
                    timeZone: 'Asia/Manila',
                    year: 'numeric',
                    month: 'long',
                    day: 'numeric',
                ...
                <p style="color: #666; margin-top: 0;">Generated by: ${formattedUsername}</p>
            </div>
            ...
            <table style="width: 100%; border-collapse: collapse; margin-bottom: 30px;">
                ...
                <tbody>
                    ${regressionResults.historicalChartData.map(item => `
                        <tr style="border-bottom: 1px solid #eee;">
                            <td style="padding: 8px 10px;">${formatDate(new Date(item.x))}</td>
                            <td style="padding: 8px 10px; text-align: right;">${formatCurrency(item.y)}</td>
                        </tr>
                    `).join('')}
                </tbody>
            </table>
            ...
            <table style="width: 100%; border-collapse: collapse; margin-bottom: 30px;">
                ...
                <tbody>
                    ${regressionResults.forecastData.map(item => `
                        <tr style="border-bottom: 1px solid #eee;">
                            <td style="padding: 8px 10px;">${formatDate(new Date(item.x))}</td>
                            <td style="padding: 8px 10px; text-align: right;">${formatCurrency(item.y)}</td>
                        </tr>
            ...
            <!-- Footer -->
            <div style="text-align: center; margin-top: 30px; border-top: 2px solid #3A57E8; padding-top: 10px; color: #666; font-size: 12px;">
                <p>Forecast Accuracy: ${forecastAccuracy.toFixed(1)}%</p>
                <p>Contact: (0956) 814-3000 | (0961) 345-4283 | Email: GrievEase@gmail.com</p>
                <p>© ${new Date().getFullYear()} VJay Relova Funeral Services. All rights reserved.</p>
            </div>
        </div>
    `;
...
window.printRevenueTables = printRevenueTables;
</script>
<script>
document.addEventListener('DOMContentLoaded', function() {
  ...
  document.getElementById('printButton').addEventListener('click', function() {
      ...
      const headerHTML = `
          <div class="header">
              ...
              <p>Generated by: ${formattedUsername}</p>
          </div>
      `;
