...
const chartData = {
    ...
    yearly: {
        data: <?php echo json_encode($yearlyRevenueData); ?>,
...
// Toggle between monthly and yearly views without page refresh
document.getElementById('cashMonthlyView').addEventListener('click', function() {
    ...
    this.classList.add('bg-[#4ade80]', 'text-white');
    this.classList.remove('text-gray-600', 'hover:text-gray-800');
    document.getElementById('cashYearlyView').classList.remove('bg-[#4ade80]', 'text-white');
    document.getElementById('cashYearlyView').classList.add('text-gray-600', 'hover:text-gray-800');
});
...
document.getElementById('cashYearlyView').addEventListener('click', function() {
    ...
    this.classList.add('bg-[#4ade80]', 'text-white');
    this.classList.remove('text-gray-600', 'hover:text-gray-800');
    document.getElementById('cashMonthlyView').classList.remove('bg-[#4ade80]', 'text-white');
    document.getElementById('cashMonthlyView').classList.add('text-gray-600', 'hover:text-gray-800');
});

// Export PDF function with clean black & white design
document.getElementById('exportCashRevenue').addEventListener('click', function() {
    ...
    const generatedBy = 'Administrator'; // You can make this dynamic based on logged user
    const reportType = isYearly ? 'Yearly Cash Revenue Report' : 'Monthly Cash Revenue Report';
    ...
    doc.text('VJAY RELOVA FUNERAL SERVICES', 105, 25, { align: 'center' });
    ...
    doc.setFontSize(16);
    doc.setFont('helvetica', 'bold');
    doc.text(reportType.toUpperCase(), 105, 40, { align: 'center' });
    
    // Date & Time Generated
    doc.setFontSize(10);
    doc.setFont('helvetica', 'normal');
    doc.text('Date Generated: ' + currentDate.toLocaleString('en-PH', {
        year: 'numeric',
        month: 'long',
        day: 'numeric',
        hour: '2-digit',
    ...
    // Generated By
    doc.text('Generated By: ' + generatedBy, 20, 56);
    ...
    // Create table with clean black and white styling
    doc.autoTable({
        head: [tableData[0]],
        body: tableData.slice(1, -1), // Exclude total row from body
        startY: 70,
        theme: 'grid',
        headStyles: {
        ...
        didDrawPage: function (data) {
            // Add total row manually with special formatting
            if (data.pageCount === data.pageNumber) {
                const finalY = data.cursor.y + 5;
                
                // Total row background (black)
                doc.setFillColor(0, 0, 0);
                doc.rect(margin, finalY, availableWidth, 15, 'F');
                
                // Total row border
                doc.setDrawColor(0, 0, 0);
                doc.setLineWidth(1);
                doc.rect(margin, finalY, availableWidth, 15, 'S');
                
                // Total text (white on black)
                doc.setFontSize(12);
                doc.setFont('helvetica', 'bold');
                doc.setTextColor(255, 255, 255);
                doc.text(tableData[tableData.length - 1][0], margin + 10, finalY + 10);
                doc.text(tableData[tableData.length - 1][1], pageWidth - margin - 10, finalY + 10, { align: 'right' });
                ...
                // Signature line
                doc.setLineWidth(0.5);
                doc.line(margin + 50, signatureY + 20, margin + 150, signatureY + 20);
                
                // Name under signature line
                doc.setFont('helvetica', 'bold');
                doc.text(generatedBy.toUpperCase(), margin + 100, signatureY + 30, { align: 'center' });
                doc.setFont('helvetica', 'normal');
                doc.text('Administrator', margin + 100, signatureY + 36, { align: 'center' });
                
                // Date line
                doc.text('Date: ________________', margin + 50, signatureY + 50);
                ...
                // Contact information
                doc.text('Vjay Relova Funeral Services | Tel: (02) 1234-5678 | Mobile: 0917-123-4567 | Email: info@vjayrelova.com',
    ...
    const fileName = `Vjay-Relova-Cash-Revenue-Report-${isYearly ? 'Yearly' : 'Monthly'}-${currentDate.toISOString().slice(0, 10)}.pdf`;
    doc.save(fileName);
});
</script>
<script>
...
// Update export functionality to handle both views with clean black & white design
document.getElementById('exportProjectedIncome').addEventListener('click', function () {
  ...
  const currentDate = new Date();
  const generatedBy = 'Administrator'; // You can make this dynamic based on logged user
  const reportType = isMonthly ? 'Monthly Accrued Revenue Report' : 'Yearly Accrued Revenue Report';
  ...
  // Save with descriptive filename
  const fileName = `Vjay-Relova-${isMonthly ? 'Monthly' : 'Yearly'}-Revenue-Report-${currentDate.toISOString().slice(0, 10)}.pdf`;
  doc.save(fileName);
});
</script>
<script>
...
document.getElementById('exportPdfBtn').addEventListener('click', function() {
  try {
    ...
    const currentDate = new Date();
    const generatedBy = 'Administrator'; // You can make this dynamic based on logged user
    const reportType = `${timeframe} Branch Revenue Report`;
    ...
    // Create table data
    const headers = [isYearly ? 'Year' : 'Month', 'Pila Branch Revenue', 'Paete Branch Revenue'];
    const body = [];
    
    // Add data rows
    for (let i = 0; i < currentData.labels.length; i++) {
      const cleanLabel = currentData.labels[i].toString()
        .replace(/±/g, '')
        .replace(/[^a-zA-Z0-9ñÑ\s]/g, '')
        .trim() || (isYearly ? `Year ${i + 1}` : `Month ${i + 1}`);
        
      body.push([
        cleanLabel,
        `PHP ${parseFloat(currentData.pila[i] || 0).toLocaleString('en-PH', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}`,
        `PHP ${parseFloat(currentData.paete[i] || 0).toLocaleString('en-PH', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}`
      ]);
    }
    ...
    const grandTotal = pilaTotal + paeteTotal;
    
    // Add totals row
    const totalsRow = [
      'TOTAL',
      `PHP ${pilaTotal.toLocaleString('en-PH', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}`,
      `PHP ${paeteTotal.toLocaleString('en-PH', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}`
    ];
    ...
    doc.autoTable({
      head: [headers],
      body: body, // Include all data rows
      startY: 70,
      theme: 'grid',
      headStyles: {
        fillColor: [0, 0, 0], // Black header
      ...
      didDrawPage: function (data) {
        ...
        if (data.pageCount === data.pageNumber) {
          ...
          // Position totals properly across the three columns
          const col1X = margin + 10;
          const col2X = margin + (availableWidth * 0.4) + (availableWidth * 0.15);
          const col3X = margin + (availableWidth * 0.4) + (availableWidth * 0.3) + (availableWidth * 0.15);
          
          doc.text(totalsRow[0], col1X, finalY + 10);
          doc.text(totalsRow[1], col2X, finalY + 10, { align: 'center' });
          doc.text(totalsRow[2], col3X, finalY + 10, { align: 'center' });
          
          // Add grand total row
          const grandTotalY = finalY + 20;
          doc.setFillColor(50, 50, 50); // Dark gray for grand total
          doc.rect(margin, grandTotalY, availableWidth, 15, 'F');
          doc.rect(margin, grandTotalY, availableWidth, 15, 'S');
          
          doc.setTextColor(255, 255, 255);
          doc.text('GRAND TOTAL', col1X, grandTotalY + 10);
          doc.text(`PHP ${grandTotal.toLocaleString('en-PH', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}`, 
                   pageWidth - margin - 10, grandTotalY + 10, { align: 'right' });
          
          // SIGNATURE SECTION
          const signatureY = grandTotalY + 40;
    ...
    // Save with descriptive filename
    const fileName = `Vjay-Relova-Branch-Revenue-Report-${timeframe}-${currentDate.toISOString().slice(0, 10)}.pdf`;
    doc.save(fileName);
