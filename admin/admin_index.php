...
// Update export functionality to handle both views with simple black & white design
document.getElementById('exportProjectedIncome').addEventListener('click', function () {
  ...
  doc.text('VJAY RELOVA FUNERAL SERVICES', centerX, currentY, { align: 'center' });
  currentY += 15;
  ...
  doc.text(reportTitle, centerX, currentY, { align: 'center' });
  currentY += 20;

  // Generation info
  doc.setFontSize(10);
  doc.setFont('helvetica', 'normal');
  doc.text(`Date Generated: ${dateGenerated}`, margin, currentY);
  currentY += 6;
  doc.text(`Generated by: ${generatedBy}`, margin, currentY);
  currentY += 20;

  // TABLE SECTION - Simple grid
  doc.autoTable({
    head: [tableData[0]],
    body: tableData.slice(1, -1), // Exclude total row
    startY: currentY,
    theme: 'grid',
    headStyles: {
    ...
    didDrawPage: function (data) {
      if (data.pageCount === data.pageNumber) {
        ...
        // TOTAL SECTION - Simple line
        doc.setLineWidth(0.5);
        doc.line(margin, tableEndY, pageWidth - margin, tableEndY);
        
        doc.setFontSize(12);
        doc.setFont('helvetica', 'bold');
        doc.text('GRAND TOTAL:', margin, tableEndY + 8);
        doc.text(tableData[tableData.length - 1][1], pageWidth - margin, tableEndY + 8, { align: 'right' });
        
        // Another line under total
        doc.line(margin, tableEndY + 12, pageWidth - margin, tableEndY + 12);

        // SIGNATURE SECTION - Simple text
        const signatureY = tableEndY + 35;
        doc.setFontSize(10);
        doc.setFont('helvetica', 'normal');
        doc.text('Prepared by:', pageWidth - 80, signatureY);
        
        // Signature line
        doc.setLineWidth(0.3);
        doc.line(pageWidth - 80, signatureY + 15, pageWidth - margin, signatureY + 15);
        
        doc.setFont('helvetica', 'bold');
        doc.text(generatedBy, pageWidth - 80, signatureY + 20);
  ...
  // Simple filename
  const filename = `Vjay-Relova-Revenue-Report-${currentDate.toISOString().slice(0, 10)}.pdf`;
  doc.save(filename);
});
</script>
<script>
