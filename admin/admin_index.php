...
for ($i = 11; $i >= 0; $i--) {
    ...
    $query = "SELECT SUM(amount_paid) as revenue FROM (
        -- 1. Direct sales from sales_tb not already in analytics_tb
        SELECT s.amount_paid
        FROM sales_tb s
        WHERE s.branch_id = 2
        AND MONTH(s.get_timestamp) = ? 
        AND YEAR(s.get_timestamp) = ?
        AND s.sales_id NOT IN (
            SELECT a.sales_id 
            FROM analytics_tb a 
            WHERE a.sales_type = 'traditional'
            AND MONTH(a.sale_date) = ? 
            AND YEAR(a.sale_date) = ?
        )
        
        UNION ALL
        
        -- 2. Direct custom sales from customsales_tb not already in analytics_tb
        SELECT c.amount_paid
        FROM customsales_tb c
        WHERE c.branch_id = 2
        AND MONTH(c.get_timestamp) = ? 
        AND YEAR(c.get_timestamp) = ?
        AND c.customsales_id NOT IN (
            SELECT a.sales_id 
            FROM analytics_tb a 
            WHERE a.sales_type = 'custom'
        ...
        -- 3. Analytics records (with preference for linked sales/customsales if they exist)
        SELECT 
            CASE
                WHEN a.sales_type = 'traditional' AND s.sales_id IS NOT NULL THEN s.amount_paid
                WHEN a.sales_type = 'custom' AND c.customsales_id IS NOT NULL THEN c.amount_paid
        ...
        FROM analytics_tb a
        LEFT JOIN sales_tb s 
            ON a.sales_type = 'traditional' 
            AND a.sales_id = s.sales_id 
            AND s.branch_id = 2
        LEFT JOIN customsales_tb c 
            ON a.sales_type = 'custom' 
            AND a.sales_id = c.customsales_id 
            AND c.branch_id = 2
        WHERE MONTH(a.sale_date) = ? 
        AND YEAR(a.sale_date) = ?
        AND a.branch_id = 2
    ) as combined_sales";
