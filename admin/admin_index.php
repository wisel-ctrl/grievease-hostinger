...
document.getElementById('exportCashRevenue').addEventListener('click', function() {
  ...
  pdfHeader(doc, 'CASH REVENUE REPORT - ' + (isYearly ? 'YEARLY' : 'MONTHLY'), '');
...
document.getElementById('exportProjectedIncome').addEventListener('click', function () {
  ...
  pdfHeader(doc, 'ACCRUED REVENUE REPORT', '(If all payments have been settled)');
  const pageWidth = doc.internal.pageSize.width;
  const margin = 15;
  const availableWidth = pageWidth - (2 * margin);
  doc.autoTable({
...
document.getElementById('exportPdfBtn').addEventListener('click', function() {
  try {
    ...
    pdfHeader(doc, timeframe.toUpperCase() + ' BRANCH REVENUE REPORT', '');
...
branchServicesChart.render();
 ...
 const services = <?php echo json_encode($services); ?>;
  ...
  try {
    ...
    pdfHeader(doc, 'SERVICE PACKAGE SALES REPORT', '');
    const tableData = [];
