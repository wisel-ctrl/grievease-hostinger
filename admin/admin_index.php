...
document.getElementById('exportProjectedIncome').addEventListener('click', function () {
  ...
  addPdfHeader(doc, 'ACCRUED REVENUE REPORT', '(If all payments have been settled)');
  const pageWidth = doc.internal.pageSize.width;
  const margin = 15;
  const availableWidth = pageWidth - (2 * margin);
  doc.autoTable({
...
document.getElementById('exportPdfBtn').addEventListener('click', function() {
  try {
    ...
    addPdfHeader(doc, `${timeframe.toUpperCase()} BRANCH REVENUE REPORT`, '');
...
document.getElementById('exportTopSellingPackages').addEventListener('click', function() {
  try {
    ...
    addPdfHeader(doc, 'SERVICE PACKAGE SALES REPORT', '');
    const tableData = [];
    ...
    doc.autoTable({
      ...
      margin: { left: margin, right: margin, top: 60 },
      didDrawPage: function(data) {
        addPdfFooter(doc, data.cursor.y + 10);
      }
    });
