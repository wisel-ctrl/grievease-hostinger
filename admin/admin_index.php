...
// Update export functionality to handle both views with enhanced UI
document.getElementById('exportProjectedIncome').addEventListener('click', function () {
  ...
  doc.text('VJAY RELOVA FUNERAL SERVICES', centerX, 18, { align: 'center' });
  
  // Decorative line under company name
  doc.setDrawColor(59, 130, 246);
  doc.setLineWidth(1);
  doc.line(centerX - 80, 22, centerX + 80, 22);
  ...
  doc.text(reportTitle, centerX, 32, { align: 'center' });
  ...
  // You can modify this to get the actual logged-in user
  ...
  doc.text('Generated on:', 20, 42);
  doc.setFont('helvetica', 'bold');
  doc.text(dateGenerated, 55, 42);
  
  // Generated by
  doc.setFont('helvetica', 'normal');
  doc.text('Generated by:', 20, 48);
  doc.setFont('helvetica', 'bold');
  doc.text(generatedBy, 55, 48);
  ...
  doc.autoTable({
    ...
    columnStyles: {
      ...
      1: { 
        ...
        halign: 'right',
        fontStyle: 'bold',
        textColor: [22, 163, 74] // Green for money values
      }
    },
    styles: {
      fontSize: 10,
      cellPadding: 5,
      lineColor: [203, 213, 225], // Light border color
      lineWidth: 0.3,
      valign: 'middle'
    },
    margin: { 
    ...
    didDrawPage: function (data) {
      if (data.pageCount === data.pageNumber) {
        ...
        doc.setFillColor(30, 41, 59); // Dark background for total
        doc.roundedRect(margin, finalY - 8, availableWidth, 16, 2, 2, 'F');
        
        // Total label and value
        doc.setFontSize(14);
        doc.setFont('helvetica', 'bold');
        doc.setTextColor(255, 255, 255); // White text
        doc.text('GRAND TOTAL:', margin + 10, finalY);
        doc.text(tableData[tableData.length - 1][1], pageWidth - margin - 10, finalY, { align: 'right' });
        ...
        doc.line(pageWidth - 120, signatureY, pageWidth - 20, signatureY);
        
        doc.setFontSize(10);
        doc.setTextColor(100, 116, 139);
        doc.setFont('helvetica', 'normal');
        doc.text('Generated by:', pageWidth - 120, signatureY + 8);
        
        doc.setFont('helvetica', 'bold');
        doc.setTextColor(30, 41, 59);
        doc.text(generatedBy, pageWidth - 120, signatureY + 15);
        
        doc.setFont('helvetica', 'italic');
        doc.setFontSize(9);
        doc.text('Digital Signature', pageWidth - 70, signatureY + 22, { align: 'center' });
