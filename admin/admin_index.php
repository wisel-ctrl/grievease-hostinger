...
document.getElementById('exportCashRevenue').addEventListener('click', function() {
    ...
    categories.forEach((category, index) => {
        ...
        tableData.push([cleanCategory, 'PHP ' + value.toLocaleString('en-PH', {minimumFractionDigits: 2, maximumFractionDigits: 2})]);
    });
    const total = revenues.reduce((sum, val) => sum + (Number(val) || 0), 0);
    tableData.push(['TOTAL', 'PHP ' + total.toLocaleString('en-PH', {minimumFractionDigits: 2, maximumFractionDigits: 2})]);
...
document.getElementById('exportPdfBtn').addEventListener('click', function() {
  try {
    const { jsPDF } = window.jspdf;
    const doc = new jsPDF({orientation: 'portrait'});
    doc.setProperties({title: 'Vjay Relova Branch Revenue Report', subject: 'Financial Report', author: 'Vjay Relova Funeral Services', keywords: 'revenue, report, financial, branch', creator: 'Vjay Relova Web Application'});
