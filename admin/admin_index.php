...
// Update export functionality to handle both views with 100% user-friendly UI
document.getElementById('exportProjectedIncome').addEventListener('click', function () {
  ...
  const dateGenerated = currentDate.toLocaleString('en-PH', {
    year: 'numeric',
    month: 'long',
    day: 'numeric',
    hour: '2-digit',
  ...
  const generatedBy = 'System Administrator'; // Replace with actual user info

  // HEADER SECTION (0-70mm from top)
  // Header background with padding
  ...
  doc.text('VJAY RELOVA FUNERAL SERVICES', centerX, 20, { align: 'center' });
  
  // Decorative line under company name
  doc.setDrawColor(59, 130, 246);
  doc.setLineWidth(0.8);
  doc.line(centerX - 60, 25, centerX + 60, 25);

  // Report title
  doc.setFontSize(16);
  doc.setTextColor(59, 130, 246);
  const reportTitle = `${isMonthly ? 'MONTHLY' : 'YEARLY'} ACCRUED REVENUE REPORT`;
  doc.text(reportTitle, centerX, 35, { align: 'center' });
  ...
  // Left side info
  doc.text('Date Generated:', margin, 50);
  doc.setFont('helvetica', 'bold');
  doc.text(dateGenerated, margin + 35, 50);
  
  doc.setFont('helvetica', 'normal');
  doc.text('Generated by:', margin, 58);
  doc.setFont('helvetica', 'bold');
  doc.text(generatedBy, margin + 30, 58);
  ...
  // TABLE SECTION (starts at 80mm with proper spacing)
  const tableStartY = 80;
  
  doc.autoTable({
    head: [tableData[0]],
    body: tableData.slice(1, -1), // Exclude total row for now
    startY: tableStartY,
    theme: 'striped',
    headStyles: {
    ...
    columnStyles: {
      ...
      1: { 
        ...
        halign: 'right',
        fontStyle: 'bold',
        textColor: [5, 150, 105] // Green for amounts
      }
    },
    styles: {
    ...
    didDrawPage: function (data) {
      // Only add total and signature on the last page
      if (data.pageCount === data.pageNumber) {
        const tableEndY = data.cursor.y;
        
        // TOTAL SECTION with proper spacing
        const totalSectionY = tableEndY + 15;
        
        // Total background
        doc.setFillColor(15, 23, 42);
        doc.roundedRect(margin, totalSectionY - 5, pageWidth - (2 * margin), 18, 2, 2, 'F');
        
        // Total text
        doc.setFontSize(14);
        doc.setFont('helvetica', 'bold');
        doc.setTextColor(255, 255, 255);
        doc.text('GRAND TOTAL:', margin + 10, totalSectionY + 5);
        doc.text(tableData[tableData.length - 1][1], pageWidth - margin - 10, totalSectionY + 5, { align: 'right' });

        // SIGNATURE SECTION with proper spacing
        const signatureSectionY = totalSectionY + 40;
        
        // Only add signature if there's enough space
        if (signatureSectionY + 30 < pageHeight - 30) {
          // Signature box
          doc.setDrawColor(203, 213, 225);
          doc.setLineWidth(0.3);
          doc.rect(pageWidth - 80, signatureSectionY, 65, 25);
          
          // Signature line
          doc.line(pageWidth - 75, signatureSectionY + 15, pageWidth - 20, signatureSectionY + 15);
          
          // Labels
          doc.setFontSize(9);
          doc.setTextColor(71, 85, 105);
          doc.setFont('helvetica', 'normal');
          doc.text('Prepared by:', pageWidth - 75, signatureSectionY + 8);
          doc.text(generatedBy, pageWidth - 75, signatureSectionY + 20);
          doc.setFont('helvetica', 'italic');
          doc.setFontSize(8);
          doc.text('(Digital Signature)', pageWidth - 47.5, signatureSectionY + 23, { align: 'center' });
        }
        ...
        doc.setFont('helvetica', 'normal');
        doc.text('For inquiries: Tel: (02) 1234-5678 • Mobile: 0917-123-4567 • Email: info@vjayrelova.com',
          centerX, footerY - 3, { align: 'center' });
        
        // Page number
        doc.text(`Page ${data.pageNumber} of ${data.pageCount}`, pageWidth - margin, footerY - 3, { align: 'right' });
      }
    }
  });
  ...
  const filename = `Vjay-Relova-${isMonthly ? 'Monthly' : 'Yearly'}-Revenue-Report-${timestamp}.pdf`;
  doc.save(filename);
});
</script>
<script>
