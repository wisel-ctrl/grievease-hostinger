...
$lifeplan_id = isset($_GET['lifeplan_id']) ? $_GET['lifeplan_id'] : null;
$reason = isset($_GET['reason']) ? $_GET['reason'] : null;

// Validate required parameters
if (!$payment_id || !$lifeplan_id || !$reason) {
...
try {
    ...
    $query = "SELECT u.phone_number, 
              ...
              FROM lifeplanpayment_request_tb pr
              JOIN lifeplan_tb lp ON pr.lifeplan_id = lp.lifeplan_id
              JOIN users u ON lp.customerID = u.id
              WHERE pr.payment_id = ? AND pr.lifeplan_id = ?";
    $stmt = $conn->prepare($query);
    $stmt->bind_param("ss", $payment_id, $lifeplan_id);
    $stmt->execute();
    ...
    $stmt = $conn->prepare("UPDATE lifeplanpayment_request_tb 
                           SET acceptdecline_date = NOW(), 
                               decline_reason = ?,
                               status = 'declined'
                           WHERE payment_id = ? AND lifeplan_id = ?");
    
    // Bind parameters and execute
    $stmt->bind_param("sss", $reason, $payment_id, $lifeplan_id);
    $stmt->execute();
