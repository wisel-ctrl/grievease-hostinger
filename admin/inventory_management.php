  ...
  while($branch = $branchResult->fetch_assoc()) {
    ...
    <div class="bg-sidebar-hover p-4 border-b border-sidebar-border flex flex-col md:flex-row md:items-center md:justify-between gap-4">
        <div class="flex items-center">
            ...
            <h4 class="text-lg font-bold text-sidebar-text">
        ...
        <div class="flex flex-col md:flex-row items-start md:items-center gap-3 w-full md:w-auto">
            ...
            <div class="relative filter-dropdown">
                <button class="px-3 py-2 border border-gray-300 rounded-lg text-sm flex items-center gap-2 hover:bg-sidebar-hover"
                    ...
                    <span>Filters</span>
                    <?php if(isset($activeFilter) && $activeFilter): ?>
                ...
                <div id="filterWindow<?php echo $branchId; ?>" class="hidden absolute right-0 mt-2 w-64 bg-white rounded-md shadow-lg z-10 border border-sidebar-border p-4">
                    <div class="space-y-4">
                        ...
                        <div>
                            ...
                            <div class="space-y-1">
                                <div class="flex items-center cursor-pointer" onclick="setSortFilter(<?php echo $branchId; ?>, 'default')">
                                    <span class="filter-option hover:bg-sidebar-hover px-2 py-1 rounded text-sm w-full">
                                        Default (Unsorted)
                                    </span>
                                </div>
                                <div class="flex items-center cursor-pointer" onclick="setSortFilter(<?php echo $branchId; ?>, 'price_asc')">
                                    <span class="filter-option hover:bg-sidebar-hover px-2 py-1 rounded text-sm w-full">
                                        Price: Low to High
                                    </span>
                                </div>
                                <div class="flex items-center cursor-pointer" onclick="setSortFilter(<?php echo $branchId; ?>, 'price_desc')">
                                    <span class="filter-option hover:bg-sidebar-hover px-2 py-1 rounded text-sm w-full">
                                        Price: High to Low
                                    </span>
                                </div>
                                <div class="flex items-center cursor-pointer" onclick="setSortFilter(<?php echo $branchId; ?>, 'quantity_asc')">
                                    <span class="filter-option hover:bg-sidebar-hover px-2 py-1 rounded text-sm w-full">
                                        Quantity: Low to High
                                    </span>
                                </div>
                                <div class="flex items-center cursor-pointer" onclick="setSortFilter(<?php echo $branchId; ?>, 'quantity_desc')">
                                    <span class="filter-option hover:bg-sidebar-hover px-2 py-1 rounded text-sm w-full">
                                        Quantity: High to Low
                                    </span>
                                </div>
                                <div class="flex items-center cursor-pointer" onclick="setSortFilter(<?php echo $branchId; ?>, 'name_asc')">
                                    <span class="filter-option hover:bg-sidebar-hover px-2 py-1 rounded text-sm w-full">
                                        Name: A to Z
                                    </span>
                                </div>
                                <div class="flex items-center cursor-pointer" onclick="setSortFilter(<?php echo $branchId; ?>, 'name_desc')">
                                    <span class="filter-option hover:bg-sidebar-hover px-2 py-1 rounded text-sm w-full">
                                        Name: Z to A
                                    </span>
                                </div>
            ...
            <!-- Archived Button -->
            <button class="px-3 py-2 bg-gray-200 text-gray-700 rounded-lg text-sm flex items-center gap-2 hover:bg-gray-300 transition-colors" 
    ...
    <div class="overflow-x-auto scrollbar-thin" id="tableContainer<?php echo $branchId; ?>">
            ...
            <thead>
                <tr class="bg-gray-50 border-b border-sidebar-border">
                    ...
                    <th class="p-4 text-left text-sm font-medium text-sidebar-text cursor-pointer" onclick="sortTable(<?php echo $branchId; ?>, 1)">
                        <div class="flex items-center">
                            <i class="fas fa-tag mr-1.5 text-sidebar-accent"></i> Item Name 
                            <i class="fas fa-sort ml-1 text-gray-400"></i>
                        </div>
                    </th>
                ...
                <?php if ($paginatedResult->num_rows > 0): ?>
                    <?php while($row = $paginatedResult->fetch_assoc()): ?>
                        <tr class="border-b border-sidebar-border hover:bg-sidebar-hover transition-colors">
                            <td class="p-4 text-sm text-sidebar-text font-medium">#INV-<?php echo str_pad($row["inventory_id"], 3, "0", STR_PAD_LEFT); ?></td>
                            <td class="p-4 text-sm text-sidebar-text"><?php echo htmlspecialchars($row["item_name"]); ?></td>
                            <td class="p-4 text-sm text-sidebar-text">
                                <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-blue-50 text-blue-700 border border-blue-100">
                                    <i class="fas fa-folder-open mr-1"></i> <?php echo htmlspecialchars($row["category_name"]); ?>
                                </span>
                            </td>
                            <td class="p-4 text-sm text-sidebar-text"><?php echo $row["quantity"]; ?></td>
                            <td class="p-4 text-sm font-medium text-sidebar-text">₱<?php echo number_format($row["price"], 2); ?></td>
                            <td class="p-4 text-sm font-medium text-sidebar-text">₱<?php echo number_format($row["total_value"], 2); ?></td>
                            <td class="p-4 text-sm">
                                <div class="flex space-x-2">
                                    <button class="p-2 bg-blue-100 text-blue-600 rounded-lg hover:bg-blue-200 transition-all tooltip" title="Edit Item" onclick="openViewItemModal(<?php echo $row["inventory_id"]; ?>)">
                                        <i class="fas fa-edit"></i>
                                    </button>
                                    <button class="p-2 bg-red-100 text-red-600 rounded-lg hover:bg-red-200 transition-all tooltip" title="Archive Item" onclick="archiveItem(<?php echo $row["inventory_id"]; ?>, <?php echo $branchId; ?>)">
                    ...
                    <tr>
                        <td colspan="7" class="p-6 text-sm text-center">
                            <div class="flex flex-col items-center">
                                ...
                                <p class="text-gray-500">No inventory items found for this branch</p>
                            </div>
                        </td>
                    </tr>
                <?php endif; ?>
        ...
        <div class="p-4 border-t border-sidebar-border flex justify-between items-center">
            <div class="text-sm text-gray-500">
                Showing <?php echo ($currentPage - 1) * $itemsPerPage + 1 . ' - ' . min($currentPage * $itemsPerPage, $totalItems); ?> 
            ...
            <div class="flex space-x-1">
                <button class="px-3 py-1 border border-sidebar-border rounded text-sm hover:bg-sidebar-hover <?php echo $currentPage <= 1 ? 'opacity-50 cursor-not-allowed' : ''; ?>" 
                        onclick="changePage(<?php echo $branchId; ?>, <?php echo $currentPage - 1; ?>)" 
