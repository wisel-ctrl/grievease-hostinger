...
<script>
// Function to open view item modal
function openViewItemModal(inventoryId) {
    ...
    fetch(`inventory/get_item_details.php?id=${inventoryId}`)
        ...
        .then(data => {
            ...
            modalContent.innerHTML = `
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div class="space-y-4">
                        ...
                        <div>
                            ...
                            <input type="text" value="#INV-${String(inventoryId).padStart(3, '0')}" 
                        ...
                        <div>
                            ...
                            <div class="relative">
                                <input type="text" id="editItemName" value="${escapeHtml(data.item_name)}" required 
                                       class="w-full px-3 py-2 bg-white border border-gray-300 rounded-lg focus:ring-1 focus:ring-sidebar-accent focus:border-sidebar-accent outline-none transition-all duration-200"
                                       oninput="validateItemName(this)">
                                <div id="itemNameError" class="text-red-500 text-xs mt-1 hidden">Item name cannot start with space or have consecutive spaces</div>
                            </div>
                        ...
                        <div>
                            ...
                            <select id="editCategory" required class="w-full px-3 py-2 bg-white border border-gray-300 rounded-lg focus:ring-1 focus:ring-sidebar-accent focus:border-sidebar-accent outline-none transition-all duration-200">
                                ${generateCategoryOptions(data.category_id)}
                            </select>
                        </div>
                        ...
                        <div>
                            ...
                            <input type="number" id="editQuantity" value="${data.quantity}" min="1" required 
                                   class="w-full px-3 py-2 bg-white border border-gray-300 rounded-lg focus:ring-1 focus:ring-sidebar-accent focus:border-sidebar-accent outline-none transition-all duration-200"
                        ...
                        <div>
                            <label class="block text-xs font-medium text-gray-700 mb-1 flex items-center">
                                Unit Price <span class="text-red-500 ml-1">*</span>
                            </label>
                            <div class="relative">
                                <div class="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none">
                                    <span class="text-gray-500">₱</span>
                                </div>
                                <input type="text" id="editUnitPrice" value="${data.price.toFixed(2)}" required 
                                       class="w-full pl-8 px-3 py-2 bg-white border border-gray-300 rounded-lg focus:ring-1 focus:ring-sidebar-accent focus:border-sidebar-accent outline-none transition-all duration-200"
                    ...
                    <div class="space-y-4">
                        ...
                        <div>
                            ...
                            <div class="bg-gray-100 rounded-lg overflow-hidden h-64 flex items-center justify-center">
                                ${data.image_path ? 
                                    `<img src="${data.image_path}" alt="Item Image" class="max-h-full max-w-full object-contain" id="currentImagePreview">` : 
                        ...
                        <!-- Update Image -->
                        <div>
                            <label class="block text-xs font-medium text-gray-700 mb-1">Update Image</label>
                            <div class="border-2 border-dashed border-gray-300 rounded-lg p-4">
                                <div class="flex flex-col items-center justify-center space-y-2">
                                    <div id="newImagePreviewContainer" class="hidden w-full">
                                        <img id="newImagePreview" src="#" alt="Preview" class="max-h-48 mx-auto rounded-lg border border-gray-300">
                                    </div>
                                    <input type="file" id="editItemImage" accept="image/*" class="hidden" onchange="previewImage(this)">
                ...
                <!-- Hidden fields -->
                <input type="hidden" id="editItemId" value="${inventoryId}">
            `;
        ...
        .catch(error => {
            ...
            modalContent.innerHTML = `
                <div class="text-center py-8">
                    ...
                    <p class="text-gray-700">Failed to load item details. Please try again.</p>
                    <button onclick="openViewItemModal(${inventoryId})" class="mt-4 px-4 py-2 bg-sidebar-accent text-white rounded-lg hover:bg-darkgold transition-colors">
                        Retry
                    </button>
                </div>
            `;
...
function generateCategoryOptions(selectedId) {
    // This would typically come from an AJAX call or PHP variable
    const categories = [
...
// Setup validation for the modal inputs
...
function saveItemChanges() {
    ...
    // Validate inputs
    if (!validateItemName(document.getElementById('editItemName'))) {
        Swal.fire({
            icon: 'error',
    ...
    // Send data via AJAX
    fetch('inventory/update_item.php', {
        method: 'POST',
        body: formData
    })
    .then(response => response.json())
    .then(data => {
        ...
        if (data.success) {
            Swal.fire({
                ...
                const branchId = document.querySelector('.branch-container').dataset.branchId;
                loadPage(branchId, 1);
            });
        } else {
            Swal.fire({
