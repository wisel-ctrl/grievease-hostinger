...
function showArchivedItems(branchId) {
  document.getElementById('archivedItemsModal').classList.remove('hidden');
    ...
    // Use AJAX to fetch archived items for the specific branch
    $.ajax({
        url: 'inventory/get_archived_items.php',
        type: 'POST',
        data: { branch_id: branchId },
        success: function(response) {
            // Get the branch name from the response
            let branchName = $('#branchTitle_' + branchId).text().trim();
            
            // Update the modal title with the branch name
            ...
            // Show the modal
            $('#archivedItemsModal').removeClass('hidden');
        },
...
// Modify your showArchivedItems function to trigger search after content loads
function showArchivedItems(branchId) {
    document.getElementById('archivedItemsModal').classList.remove('hidden');
    document.body.style.overflow = 'hidden';
