...
if (!empty($search)) {
    $query .= " AND (CONCAT(u.first_name, ' ', u.last_name) LIKE ? OR s.service_name LIKE ? OR b.booking_id LIKE ?)";
    $searchParam = "%$search%";
}
...
if (!empty($search)) {
    $countQuery .= " AND (CONCAT(u.first_name, ' ', u.last_name) LIKE ? OR s.service_name LIKE ? OR b.booking_id LIKE ?)";
}
...
if ($result->num_rows > 0) {
    while ($row = $result->fetch_assoc()) {
        ...
        $html .= 'title="View Details" onclick="openBookingDetails(' . $row['booking_id'] . ')">';
    ...
    $html .= '<p class="text-gray-500">No pending bookings found</p></div></td></tr>';
}
...
$response = [
    ...
    'paginationInfo' => $total_bookings > 0 ? 
        "Showing " . ($offset + 1) . " - " . min($offset + $bookings_per_page, $total_bookings) . " of $total_bookings bookings" : 
        "No bookings found",
